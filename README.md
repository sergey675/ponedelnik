...
Этот код выполняет загрузку данных GeoJSON из файла, определяет тип геометрии, извлекает координаты, вычисляет площадь и находит ближайший объект к заданной точке, выводя результаты на консоль.
...
библиотека предоставляет удобные методы для работы с GeoJSON-данными: их загрузка из файлов, сохранение, извлечение информации, вычисление различных метрик и проведение геометрических операций.


РАЗРАБОТЧИКИ

Этот проект был разработан студентами группы ИСП-8: Поздняков Сергей и Карабеков Дияр. 
